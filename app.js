...
function setupPlayer() {
  ...
  audio.addEventListener("play", () => {
    ...
    // Once the Full Player has been opened at least once, the
    // existing <audio> element's output is permanently routed
    // through the visualizer's AudioContext (see below), so audio
    // stays silent unless that context is running — regardless of
    // whether the Full Player is currently open or closed. Keep it
    // resumed on every play, from any section.
...
function openFullPlayer() {
  ...
  visualizerOnPlayerOpen();
}

function closeFullPlayer() {
  playerOverlay.classList.add("hidden");
  visualizerOnPlayerClose();
}
...
/* =========================================================
   FULL PLAYER — AUDIO-REACTIVE VISUALIZER

   Reuses the existing <audio> element only. Creates exactly one
   AudioContext, one MediaElementSource and one AnalyserNode the
   first time the Full Player is opened, then reuses them for the
   rest of the session — never a second Audio element, never a
   second audio graph, never audio.src changes, never a playback
   restart. If Web Audio API is unavailable or creation fails for
   any reason, playback continues completely unaffected and the
   bars just stay idle.
   ========================================================= */
...
let vizSource = null;
let vizFrequencyData = null;
let vizRafId = null;
let vizBarEls = null;
let vizUnavailable = false;

function ensureVisualizerGraph() {
  if (vizUnavailable || vizAnalyser) return vizAnalyser;
  ...
  try {
    ...
    // Captures the existing <audio> element's output. Its audio must
    // be explicitly reconnected to destination below, or playback
    // would go silent through this node.
    ...
    vizFrequencyData = new Uint8Array(vizAnalyser.frequencyBinCount);
  } catch (error) {
    console.error("Visualizer unavailable:", error);
    vizUnavailable = true;
    vizAnalyser = null;
  }
...
function getVizBarEls() {
  if (!vizBarEls) {
    vizBarEls = Array.from(
      document.querySelectorAll("#playerVisualizer .viz-bar")
    );
  }
  return vizBarEls;
}

// One continuous rAF loop, started when the Full Player opens and
// stopped when it closes. Bars ease toward a target height each
// frame (from real frequency data while playing, or back down to a
// resting height while paused/unavailable) instead of snapping, so
// pausing/resuming feels like a graceful calm-down rather than a cut.
function drawVisualizerFrame() {
  vizRafId = requestAnimationFrame(drawVisualizerFrame);

  const bars = getVizBarEls();
  if (!bars.length) return;

  // Cheap per-frame watchdog: some browsers can re-suspend an
  // inactive AudioContext on their own. Since audio now depends on
  // this context staying "running" to be heard at all, keep nudging
  // it while the Full Player is open and something is playing.
  if (!audio.paused) {
    resumeVisualizerContext();
  }
  ...
  if (!vizAnalyser || audio.paused) {
    bars.forEach(bar => {
      const current = parseFloat(bar.dataset.h || String(REST_HEIGHT));
      const next = current + (REST_HEIGHT - current) * 0.18;
      bar.dataset.h = String(next);
      bar.style.height = `${next}px`;
    });
    return;
  }

  vizAnalyser.getByteFrequencyData(vizFrequencyData);

  const bins = vizFrequencyData.length;
  const perBar = Math.max(1, Math.floor(bins / bars.length));

  bars.forEach((bar, i) => {
    let sum = 0;
    const start = i * perBar;

    for (let j = start; j < start + perBar && j < bins; j++) {
      sum += vizFrequencyData[j];
    }

    const avg = sum / perBar; // 0-255
    const target = REST_HEIGHT + (avg / 255) * 14; // 4px..18px
    const current = parseFloat(bar.dataset.h || String(REST_HEIGHT));
...
function visualizerOnPlayerOpen() {
...
// Resumes the visualizer's AudioContext if (and only if) it already
// exists and isn't running. No-op until the Full Player has been
// opened at least once (that's the only place the context gets
// created), so this is always safe to call from anywhere, including
// playback paths that have nothing to do with the visualizer.
function resumeVisualizerContext() {
  if (vizAudioCtx && vizAudioCtx.state !== "running") {
    vizAudioCtx.resume().catch(() => {});
  }
}

function visualizerOnPlayerClose() {
  if (vizRafId) {
