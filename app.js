...
function showToast(message) {
  ...
  // Restart the transition even if a toast is already showing, so
  // rapid actions replace the message instead of stacking up.
...
function bindSongButtons(container) {
  ...
  container.querySelectorAll("[data-action]").forEach(button => {
    button.addEventListener("click", event => {
      ...
      if (action === "menu") openSongActionsMenu(song);
    });
  });
}
...
function openSongActionsMenu(song) {
  if (!song) return;
...
// state.favorites is the single source of truth for every visible
// Favorite indicator. Whenever it changes (from the Track List
// menu, the Mini Player, or the Full Player) this re-renders every
// surface that reflects it, so all three always agree immediately
// — no page reload, no extra song fetch.
...
async function deleteSong(song) {
  ...
  try {
    ...
    showToast(error.message || "Couldn't delete song.");
  }
}

// Forward now opens Telegram's own native chat-picker (inline
// mode) instead of having the bot send the file directly. The
// user picks the recipient themselves in Telegram's UI; our
// Worker only answers the resulting inline query (see
// handleInlineQuery in the Worker) with the song's existing
// telegram_file_id — nothing is sent by the bot on our behalf,
// and no custom caption/title is added.
function forwardSong(song) {
  if (!song?.id) return;

  if (!tg || typeof tg.switchInlineQuery !== "function") {
    showToast("Unable to open Telegram sharing");
    return;
  }
  ...
  list.querySelectorAll("[data-add-playlist-id]").forEach(button => {
    button.addEventListener("click", async () => {
      ...
      try {
        ...
        showToast(
          name
            ? `Added to ${name.length > 24 ? `${name.slice(0, 24)}…` : name}`
            : "Added to playlist"
        );
      } catch (error) {
        alert(error.message);
