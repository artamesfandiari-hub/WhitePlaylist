...
// Restarts the lightweight cover crossfade (see .cover-pulse in style.css)
// whenever the current track changes, so the cover doesn't jump-cut.
function pulseCoverArt() {
  ["miniCover", "playerCover"].forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;

    el.classList.remove("cover-pulse");
    void el.offsetWidth; // restart the CSS animation
    el.classList.add("cover-pulse");
  });
}
