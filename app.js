const API =
  "white-playlist-api-v2.mahantem2.workers.dev";
...
loadVisibleSongCovers(container);
}
...
>
  <span class="music-unicode" aria-hidden="true">𝄞</span>
</button>
...
async function loadVisibleSongCovers(container) {
  if (!container) return;

  const items =
    container.querySelectorAll(".song-item");

  for (const item of items) {
    const songId =
      Number(item.dataset.songId);
    ...
    const cover =
      item.querySelector(".song-cover");

    if (!cover) continue;

    loadSongCover(song, cover);
  }
}

async function loadSongCover(song, element) {
  if (!song?.id || !element) return;
  ...
  try {
    ...
    image.className = "song-cover-image";
    image.alt = "";
    ...
    // Keep the existing default music symbol.
    console.debug(
      "Song cover unavailable:",
      song.id
    );
  }
}

function updatePlayerCovers(song) {
  const miniCover =
  ...
  if (miniCover) {
    setDefaultCover(miniCover, "mini");
    loadPlayerCover(song, miniCover, "mini");
  }

  if (playerCover) {
    setDefaultCover(playerCover, "player");
    loadPlayerCover(song, playerCover, "player");
  }
}

function setDefaultCover(element, type) {
  if (!element) return;

  if (type === "player") {
    element.innerHTML =
      `<span class="player-cover-symbol" aria-hidden="true">𝄞</span>`;
  } else {
    element.innerHTML =
      `<span class="music-unicode" aria-hidden="true">𝄞</span>`;
  }
...
async function loadPlayerCover(song, element, type) {
  if (!song?.id || !element) return;
